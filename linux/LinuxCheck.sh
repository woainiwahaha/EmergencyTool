...
echo ""
echo " ========================================================= "
echo " \        Linux应急处置/信息搜集/漏洞检测脚本 100 Pro Max      / "
echo " ========================================================= "
echo " # 支持Centos、Debian系统检测                    "
echo " # 更新日期：2025年10月29日                    "
echo " # 参考来源：                "
...
echo " #   Ubuntu/CentOS 通用运行（不安装依赖）:bash LinuxCheck.sh --no-install"
echo " #   离线模式（跳过安装与上传）: bash LinuxCheck.sh --offline"
echo " #   需要扫描 Web 目录时:bash LinuxCheck.sh --webpath /var/www"
echo -e "\n"

# WEB Path 设置web目录，检测Webshell（默认不扫描，需 --webpath 指定目录）。
webpath=''
...
# 设置保存文件
# 优先使用 ip 命令获取 IPv4 地址，兼容不同发行版
ipaddress=$(ip -4 addr show 2>/dev/null | awk '/inet /{print $2}' | cut -d/ -f1 | egrep -v '^127\.' | head -n1)
if [ -z "$ipaddress" ]; then
  ipaddress=$(ifconfig 2>/dev/null | grep -E -o 'inet (addr:)?([0-9]{1,3}\.){3}[0-9]{1,3}' | grep -E -o '([0-9]{1,3}\.){3}[0-9]{1,3}' | egrep -v '^127\.' | head -n1)
fi
filename=$ipaddress'_'$(hostname)'_'$(whoami)'_'$(date +%s)_log'.md'

# 解析参数：--offline 跳过联网安装与上传；--webpath 指定Web目录；--no-install 不安装依赖
OFFLINE=0
NO_INSTALL=0
while [ $# -gt 0 ]; do
  case "$1" in
...
reverse_shell_check() {
  echo -e "\n"
  print_code "$(grep -E '(tftp\s\-i|scp\s|sftp\s|bash\s\-i|nc\s\-e|sh\s\-i|wget\s|curl\s|exec|/dev/tcp/|/dev/udp/)' $1 $2 $3)"
  print_code "$(grep -E '(useradd|groupadd|chattr|fsockopen|socat|base64|socket|perl|openssl)' $1 $2 $3)"
}
...
if [ "$OFFLINE" -eq 1 ] || [ "$NO_INSTALL" -eq 1 ]; then
  print_msg "离线模式启用：跳过依赖安装"
else
...
network_check() {
  ...
  if command -v ss >/dev/null 2>&1; then
    print_code "$(ss -tulpen)"
  fi
  if command -v netstat >/dev/null 2>&1; then
    print_code "$(netstat -tulpen | grep -E 'tcp|udp.*')"
  fi

  #对外开放端口
  print_msg "### 对外开放端口"
  if command -v netstat >/dev/null 2>&1; then
    print_code "$(netstat -tulpen | awk '{print $1,$4}' | grep -E -o '.*0.0.0.0:([0-9]+)|:::[0-9]+')"
  fi
  ...
  print_msg "**TCP连接**"
  if command -v ss >/dev/null 2>&1; then
    print_code "$(ss -tan state established)"
  fi
  if command -v netstat >/dev/null 2>&1; then
    print_code "$(netstat -antop | grep -E ESTAB)"
  fi
  print_msg "**UDP连接**"
  ...
  if command -v ip >/dev/null 2>&1; then
    ...
    print_code "$(/sbin/route -nee 2>/dev/null)"
  fi
  ...
  print_code "$(grep -E -o '([0-9]{1,3}\.){3}[0-9]{1,3}' </etc/resolv.conf)"
...
crontab_check() {
  ...
  # 若为 root，检查所有可登录用户的 Crontab
  if [ "$UID" -eq 0 ]; then
    print_msg "### 所有可登录用户的 Crontab"
    user_list=$(awk -F: '($7 !~ /(\/sbin\/nologin|\/usr\/sbin\/nologin|\/bin\/false)$/){print $1}' /etc/passwd)
    for u in $user_list; do
      print_msg "User: ${u}"
      ucron=$(crontab -u "$u" -l 2>/dev/null | egrep -v '^#|^$')
      if [ -n "$ucron" ]; then
        print_code "$ucron"
      else
        print_code "无 crontab"
      fi
    done
  fi
...
env_check() {
  ...
  if [ -e "${preload}" ]; then
    ...
    print_code "$(cat ${preload})"
  fi
  # 正在运行的环境变量
  print_msg "### 正在运行的进程环境变量问题"
  print_code "$(grep -E 'LD_PRELOAD|LD_ELF_PRELOAD|LD_AOUT_PRELOAD|PROMPT_COMMAND|LD_LIBRARY_PATH' /proc/*/environ 2>/dev/null)"
}
...
user_check() {
  ...
  print_msg "### 登陆ip"
  print_code "$(grep -i -a Accepted /var/log/secure /var/log/auth.* 2>/dev/null | grep -E -o '([0-9]{1,3}\.){3}[0-9]{1,3}' | sort | uniq)"
...
bash_check() {
  ...
  if echo test | grep -P 'test' >/dev/null 2>&1; then
    ...
    print_code "$(cat ~/.*history | egrep -i 'http://|https://|ssh|scp|wget|curl|nc|telnet|bash|sh|chmod|chown|/etc/passwd|/etc/shadow|/etc/hosts|nmap|frp|nfs|sshd|modprobe|lsmod|sudo|mysql|mysqldump|\.tar' | egrep -v 'man\b|ag\b|cat\b|sed\b|git\b|docker\b|rm\b|touch\b|mv\b|\bapt\b|\bapt-get\b')"
  fi
...
file_check() {
  ...
  print_msg "### ...隐藏文件"
  print_msg "$(find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" ! -path "/boot/*" ! -path "/dev/*" ! -path "/snap/*" ! -path "/var/lib/docker/*" -name ".*.")"
  ...
  #SGID
  print_msg "### SGID"
  print_code "$(find / ! -path "/proc/*" -perm -002000 -type f | egrep -v 'snap|docker')"
...
ssh_check() {
  ...
  if [ -e "/etc/ssh/sshd_config" ]; then
    print_msg "### SSHD 配置风险项"
    print_code "$(egrep -i '^(PermitRootLogin|PasswordAuthentication|PermitEmptyPasswords|PubkeyAuthentication|AuthorizedKeysFile|AllowUsers|AllowGroups)\b' /etc/ssh/sshd_config | egrep -v '^#')"
  fi
...
webshell_check() {
  ...
  if [ -z "$webpath" ]; then
    print_msg "未提供 --webpath，已跳过 Web 目录扫描"
    return
  fi
